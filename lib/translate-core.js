...
async function callGemini(prompt, geminiApiKey, publicAppUrl) {
  ...
  // No retries on 429 — those just chew through the per-minute quota
  // faster. One transient retry on 503 (server overload) is fine.
  // Netlify Free's 10s function ceiling means we can't afford the
  // 1.5s + 3s + 4.5s backoff stack the original code had.
  const maxRetries = 1;
  let lastError;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        // Retry only on 503 (transient overload). 429 means quota — retrying
        // makes it worse. 500 is rare and usually permanent for that input.
