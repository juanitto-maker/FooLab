...
// Map the AI's category enum (drink|solid|snack|dairy|meat|baked|frozen|
// condiment|other — see PROMPT.md) to the catalog's coarse food/drink split.
// Falls back to keyword sniff on the product name so misclassified rows
// still land in roughly the right tab.
function deriveKind(result) {
  const cat = String(result.category || '').toLowerCase();
  if (cat === 'drink') return 'drink';
  const hay = `${cat} ${result.productName || ''}`.toLowerCase();
  if (/\b(drink|beverage|soda|cola|juice|water|tea|coffee|beer|wine|spirits|cocktail|smoothie|milk|kombucha|lemonade|nectar|tonic|lager|ale|cider|champagne|prosecco|vodka|whisky|gin|rum|tequila)\b/.test(hay)) {
    return 'drink';
  }
  return 'food';
}
